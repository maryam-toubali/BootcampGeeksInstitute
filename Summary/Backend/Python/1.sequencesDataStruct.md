...
⚠⚠⚠ Séquence au sens Python (officiel, strict):
        * Une séquence est un type qui a : un ordre garanti / des indices / le slicing
        * Les séquences Python sont : list, tuple, str, range, bytes, bytearray. ❌ set n’est PAS une séquence ❌ dict n’est PAS une séquence
		* ✂️ slicing(découpage) en Python : Extraire une partie d’une séquence.
		 Sytaxe: sequence[start(inclus) : stop(exclu) : step(optionnel, par défaut = 1)]
