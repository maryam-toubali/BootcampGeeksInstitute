...
- Une séquence est un type qui a : un ordre garanti(les éléments sont ordonnés) / des indices(on peut y accéder par indice) / le slicing(extraire une partie d’une séquence)
- Les séquences Python sont : list, tuple, str, range, bytes, bytearray. ❌ set n’est PAS une séquence ❌ dict n’est PAS une séquence
- ✂️ slicing(découpage): > sequence[start(inclus) : stop(exclu) : step(optionnel, par défaut = 1)]
...
> bytes(Read-only binary data) et bytearray(Editable binary data) sont des séquences d’entiers (0–255)
...
✅ General Functions:
| Function          | Applies to                  | What it Does                              |
|-------------------|-----------------------------|-------------------------------------------|
| `len(sq)`         | all                         | Get the number of items                   |
| `max(sq)`         | all                         | Highest value / item                      |
| `min(sq)`         | all                         | Lowest value / item                       |
| `sum(sq)`         | all (except strings)        | Sum of all values                         |
| `sorted(sq)`      | all (except sets)           | Return a sorted copy                      |
| `enumerate(sq)`   | all                         | Get index and item together               |
...
| `index(x)`      | Get index of item         | ✅   | ❌  | ✅    |
| `count(x)`      | Count occurrences of x    | ✅   | ❌  | ✅    |
| `copy()`        | Shallow copy              | ✅   | ✅  | ❌    |
| `extend([x,y])` | Add another list          | ✅   | ❌  | ❌    |
...
| `.join()`    | `"sep".join(['a','b','c'])` → `"aSepbSepc"`  | Join sequence to string  |
